...
export async function calcLoadoutStats(ds: DataStore, miningLoadout: MiningLoadout): Promise<AllStats> {
  ...
  const activeGadget =
    miningLoadout.activeGadgetIndex > -1 && miningLoadout.inventoryGadgets.length > miningLoadout.activeGadgetIndex
  ...
  const retVal = {
    ...
    overchargeRate: multiplyReduceLasers(laserStats, 'overchargeRate') * gadgetStats.overchargeRate,
    clusterMod: multiplyReduceLasers(laserStats, 'clusterMod') * gadgetStats.clusterMod,
    inertMaterials: multiplyReduceLasers(laserStats, 'inertMaterials') * gadgetStats.inertMaterials,

    instability: multiplyReduceLasers(laserStats, 'instability') * gadgetStats.instability,
    resistance: multiplyReduceLasers(laserStats, 'resistance') * gadgetStats.resistance,
    optimalChargeRate: multiplyReduceLasers(laserStats, 'optimalChargeRate') * gadgetStats.optimalChargeRate,
    optimalChargeWindow: multiplyReduceLasers(laserStats, 'optimalChargeWindow') * gadgetStats.optimalChargeWindow,
    shatterDamage: multiplyReduceLasers(laserStats, 'shatterDamage') * gadgetStats.shatterDamage,
